...
export default function TeachersSection({ teachers, onAskSira, showViewAllButton = false }: TeachersSectionProps) {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredTeachers = useMemo(() => {
    // Search is only active on the dedicated teachers page
    if (showViewAllButton) return teachers;
    
    return teachers.filter(teacher =>
      teacher.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      teacher.subject.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [searchTerm, teachers, showViewAllButton]);
  ...
  return (
    <section id="guru-section" className="py-24 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Only show search on the dedicated teachers page */}
        {!showViewAllButton && (
        ...
        {showViewAllButton && (
          <div className="mt-16 text-center scroll-reveal">
            <Link to="/guru" className="px-8 py-4 btn-gradient font-bold rounded-3xl text-lg btn-glow transform hover:scale-105 transition-transform duration-300 inline-block">
                Lihat Semua Guru
            </Link>
          </div>
        )}
